        ...
        <>
            ...
            <div className="text-center py-4 main mb-5 busesothers">
                <MDBContainer>
                    <Fade triggerOnce>
                        <section>
                            <h3 className="py-3">{router.locale === "en" ? ("Other informations:") : ("Egyéb tudnivalók:")}</h3>
                            <p className="font-weight-bolder">{router.locale === "en" ?
                            ...
                            <p>{router.locale === "en" ? ("In addition to the indicated kilometer or hourly rate, VAT, tolls, crossing fees, city tickets, parking and a daily fee for drivers must be paid.") :
                                ...
                                {router.locale === "en" ? (" The accommodation of the driver(s) is provided by the customer, the amount of the daily fee varies by road types.") :
                                    (" Gépkocsivezető(-k) szállásáról a megrendelő gondoskodik, a napidíj összege úttípusonként változik.")}
                                {router.locale === "en" ? (" The drivers should rest 9 hours or more.") :
                                    (" A gépkocsivezetők napi pihenőideje semmilyen körülmények között sem lehet kevesebb kilenc egymást követő óránál.")}
                                {router.locale === "en" ? (" We provide preliminary information on tolls, ferry costs and parking.") :
                                    (" Autópályadíjakról, kompköltségekről, parkolásról előzetes tájékoztatást adunk.")}
                            </p>
                            <p className="font-weight-bolder m-0 p-0">{router.locale === "en" ? ("In case of regular order we provide a discount!") :
                                ("Rendszeres megrendelés esetén kedvezményt biztosítunk!")}</p>
                            <p>{router.locale === "en" ? ("Orders can be placed in person at our office, by letter, fax, e-mail or phone.") :
            ...
            <MDBContainer className="text-center my-5 px-lg-0" id="buses-container">
                ...
                <Fade triggerOnce>
                    <MDBBtn color="dark" outline onClick={() => setshowmuzeum(!showmuzeum)} className="my-1 roundedbtn muzeumbtn">
                        {showmuzeum ? router.locale === "en" ? ("« Current buses") : ("« Jelenlegi autóbuszaink") : router.locale === "en" ? ("Autobus museum »") : ("Autóbusz múzeumunk »")}
                    </MDBBtn>
                </Fade>
